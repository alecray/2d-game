...
## Set by the dropping enemy before the node is added to the tree.
var spell_id: String = ""
...
func _on_body_entered(body: Node2D) -> void:
	...
	if not stats.has_spell(spell_id):
		stats.unlock_spell(spell_id)

	var spell_name: String = SPELL_NAMES.get(spell_id, spell_id.to_upper())
